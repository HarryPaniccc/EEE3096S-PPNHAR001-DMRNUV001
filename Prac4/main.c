...
uint32_t TIM2_Ticks = (int)round(TIM2CLK/(NS*F_SIGNAL)); // The amount of clock ticks that we stay on one LUT value
uint32_t DestAddress = (uint32_t) &(TIM3->CCR3); // Write LUT TO TIM3->CCR3 to modify PWM duty cycle

uint32_t previousTick = 0;

char mode[3][10] = {"SINE","SAWTOOTH", "TRIANGULAR"};
...
int main(void)
{
  ...
  // Starting it makes it
  // enabling allows it to interrupt
  // ENABLE EACH TIME

  // TODO: Write current waveform to LCD ("Sine")
  delay(3000);
  lcd_command(CLEAR);
  lcd_putstring(mode[modecounter]);

  // TODO: Enable DMA (start transfer from LUT to CCR)
  __HAL_TIM_ENABLE_DMA(&htim2, TIM_DMA_CC1); //Seems strange to me that this is how this is called
...
void EXTI0_1_IRQHandler(void)
{
	...
	if(HAL_GetTick() - previousTick < 300) //Not long enough passed
	{
		HAL_GPIO_EXTI_IRQHandler(Button0_Pin); // Clear interrupt flags
		return;							   //flee
	}
	previousTick = HAL_GetTick();//Time is cool, so new reference
	delay(3000);
	lcd_command(CLEAR);
	...
	if(modecounter == 0) //if 0, ie it is Sine Wave, go to SAW
	{
		modecounter++;
		lcd_putstring(mode[modecounter]);
	...
	if(modecounter == 1) //if on 1, ie saw, go to tri
	{
		modecounter++;
		lcd_putstring(mode[modecounter]);
	...
	if(modecounter == 2) //if on 2, ie tri, go to sine
	{
		modecounter = 0;
		lcd_putstring(mode[modecounter]);
