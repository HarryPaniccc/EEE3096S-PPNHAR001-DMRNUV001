...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define MEM_1 0b000000000000001
#define MEM_2 0b000000000000010
#define MEM_3 0b000000000000011
#define MEM_4 0b000000000000100
#define MEM_5 0b000000000000101
#define MEM_6 0b000000000000110
uint16_t MEM_0 = 0b000000000000000;
uint8_t ghost = 0; //counter variable for time change
...
int main(void)
{
  ...
  write_to_address(MEM_1,patterns[0]);
  write_to_address(MEM_2,patterns[1]);
  write_to_address(MEM_3,patterns[2]);
  write_to_address(MEM_4,patterns[3]);
  write_to_address(MEM_5,patterns[4]);
  write_to_address(MEM_6,patterns[5]);
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	 if(HAL_GPIO_ReadPin(GPIOA, Button0_Pin) == 0)
			{
				if(ghost == 0)
				{
					MX_TIM16_Init();
...
/* USER CODE BEGIN 4 */
void PAT1(void) //turning LEDS per pattern
{
	Pin_Reset();
	LL_GPIO_TogglePin(LED7_GPIO_Port, LED7_Pin);
	LL_GPIO_TogglePin(LED5_GPIO_Port, LED5_Pin);
	LL_GPIO_TogglePin(LED3_GPIO_Port, LED3_Pin);
	LL_GPIO_TogglePin(LED1_GPIO_Port, LED1_Pin);//ONES

}

void PAT2(void) //turning LEDS per pattern
{
	Pin_Reset();
...
void PAT3(void) //turning LEDS per pattern
{
	Pin_Reset();
	LL_GPIO_TogglePin(LED7_GPIO_Port, LED7_Pin);
	LL_GPIO_TogglePin(LED6_GPIO_Port, LED6_Pin);
...
void PAT4(void) //turning LEDS per pattern
{
	Pin_Reset();
	LL_GPIO_TogglePin(LED5_GPIO_Port, LED5_Pin);
	LL_GPIO_TogglePin(LED4_GPIO_Port, LED4_Pin);
...
void PAT5(void) //turning LEDS per pattern
{
	Pin_Reset();
	LL_GPIO_TogglePin(LED7_GPIO_Port, LED7_Pin);
	LL_GPIO_TogglePin(LED6_GPIO_Port, LED6_Pin);
...
void PAT6(void) //turning LEDS per pattern
{
	Pin_Reset();
	LL_GPIO_TogglePin(LED3_GPIO_Port, LED3_Pin);
	LL_GPIO_TogglePin(LED2_GPIO_Port, LED2_Pin);
...
void Pin_Reset(void) //resets all the LED output pins to 0.
{
...
void MX_TIM16_Init_0_5(void)
{
  ...
  /* USER CODE END TIM16_Init 1 */
  htim16.Instance = TIM16;
  htim16.Init.Prescaler = 0.5*(8000-1);
  htim16.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim16.Init.Period = (1000-1);
  htim16.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
  htim16.Init.RepetitionCounter = 0;
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t pat = read_from_address(MEM_0+1);//gets pattern from memory
	...
	{
		PAT5();
	} else if (pat == patterns[5])
	{
		PAT6();
	} else
	{
		LL_GPIO_TogglePin(LED0_GPIO_Port, LED0_Pin); //incase of SPI failure
